...
|--------|----------|--------------------|
| `PIC_API_KEY` | Railway env, Modal `pic-env` | Quarterly |
| `PIC_DATABASE_URL` | Railway env, Modal `pic-env` | On compromise |
| `PIC_S3_ACCESS_KEY_ID` | Railway env, Modal `pic-env` | Quarterly |
| `PIC_S3_SECRET_ACCESS_KEY` | Railway env, Modal `pic-env` | Quarterly |
| `PIC_GDRIVE_SERVICE_ACCOUNT_JSON` | Modal `pic-env` | Annually |
| `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` | GitHub Actions secrets | Quarterly |
| `PIC_SENTRY_DSN` | Railway env | On compromise |
...
1. Generate a new key:
   ...
   ```
2. Update Railway: `railway variables set PIC_API_KEY=<new-key>`
...
3. Update Railway: `railway variables set PIC_DATABASE_URL=<new-url>`
...
2. Update Railway:
   ```bash
   railway variables set PIC_S3_ACCESS_KEY_ID=<new-id>
   railway variables set PIC_S3_SECRET_ACCESS_KEY=<new-secret>
   ```
3. Update Modal:
   ```bash
