...
- **H-S2** — #232 (internal)
- **H-P1** — #233 (internal)
- **M-P1 (promoted to high in issue tracking)** — #234 (internal)
- **H-P3** — #235 (internal)
- **H-B1** — #236 (internal)
- **H-API1** — #238 (internal)
...
### C1: Clustering destroys all L1/L2 data without transaction safety ✅ Resolved (#227 (internal))
- **File**: `src/pic/services/clustering_pipeline.py:61-96`
...
### C2: O(n²) cosine distance matrix in L1 clustering ✅ Resolved (#228 (internal))
- **File**: `src/pic/services/clustering.py`
...
### C3: HNSW index rebuild locks table (migration 015) ✅ Resolved (#229 (internal))
- **File**: `src/pic/migrations/versions/015_tune_hnsw_index_params.py:25-29`
...
### C4: SET LOCAL statement_timeout reset after commit ⚡ Quick Win ✅ Resolved (#230 (internal))
- **File**: `src/pic/worker/helpers.py:197`
...
### Security
- **H-S1**: API key exposed in URL query parameter (`api/clusters.py:267`) — browser history, proxy logs, Referer headers. Fix: use session tokens. Effort: medium. ✅ Resolved (#231 (internal))
- **H-S2**: No Pillow decompression bomb protection (`image_store.py:111`, `embedding.py:59`) — 20MB PNG → billions of pixels → OOM. Fix: set `MAX_IMAGE_PIXELS`. Effort: small ⚡. ✅ Resolved (#232 (internal))

### Performance
- **H-P1**: N+1 embedding load in product endpoints (`api/products.py:215-241`) — `db.refresh(product, ["images"])` loads 768-dim vectors just to count. Fix: use COUNT query. Effort: small ⚡. ✅ Resolved (#233 (internal))
...
- **H-P3**: Connection pool under-sized (`core/database.py`) — `pool_size=5 + max_overflow=10` insufficient. Fix: increase defaults. Effort: small ⚡. ✅ Resolved (#235 (internal))
...
### Backend Logic
- **H-B1**: GDrive cron spawns worker without checking existing running jobs (`modal_app.py:105-129`) — wastes GPU cold starts on advisory lock failures. Fix: pre-check PENDING/RUNNING jobs. Effort: small ⚡. ✅ Resolved (#236 (internal))
...
### API Design
- **H-API1**: ErrorDetail vs ProblemDetail mismatch — OpenAPI spec declares `ErrorDetail`, actual responses use `ProblemDetail`. Effort: small ⚡. ✅ Resolved (#238 (internal))
- **H-API2**: DELETE product doesn't clearly handle image orphaning (`api/products.py:245-256`). Effort: small

### DevOps
- **H-DO1**: No `permissions` block on CI workflows (`ci-cd.yml`, `deploy-staging.yml`) — overly broad token permissions. Effort: small ⚡. ✅ Resolved (#237 (internal))
- **H-DO2**: Dockerfile uses unpinned `uv:latest` (`Dockerfile:4`) — breaking UV change silently breaks builds. Effort: small ⚡. ✅ Resolved (#237 (internal))
- **H-DO3**: No rollback mechanism for Modal deployments — broken deploy shows green CI. Effort: medium. ✅ Resolved (#237 (internal))

### Testing
- **H-T1**: 5 modules with zero test coverage — cluster_visualization (148 LOC), rate_limit, metrics, image_processing (146 LOC), modal_app. Effort: medium. ✅ Resolved (#239 (internal))

## Medium-Severity Issues

### Performance (8 medium)
- **M-P1**: Presigned URL cache has no TTL — serves expired URLs (flagged by 4 agents!) (`image_store.py:170-187`). Effort: small ⚡. ✅ Resolved (#234 (internal))
...
- **M-S3**: Browser router bypasses API key dependency (`api/router.py:23-24`). Effort: small. ✅ Resolved (#231 (internal))
...
- **M-D4**: Missing `pool_timeout` configuration (`core/database.py:50-58`). Effort: small. ✅ Resolved (#235 (internal))
